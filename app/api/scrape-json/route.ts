...
async function fetchLeaderboardPage(
    ...
    const headers = { ...HEADERS, "Referer": `https://www.hackerrank.com/contests/${contestSlug}/leaderboard` };
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!contestSlug) {
            return NextResponse.json({ error: 'Missing required field: contestSlug' }, { status: 400 });
        }
        ...
        const stream = new ReadableStream({
            async start(controller) {
                try {
                    ...
                    if (!firstPage) {
                        controller.enqueue(
                            encoder.encode(JSON.stringify({ type: 'error', message: 'Failed to fetch first page' }) + '\n')
                        );
                        controller.close();
                        return;
                    }

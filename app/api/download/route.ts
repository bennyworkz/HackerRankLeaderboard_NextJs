...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!contestSlug) {
            return NextResponse.json({ error: 'Missing contestSlug parameter' }, { status: 400 });
        }

        console.log(`Starting direct download for contest: ${contestSlug}`);
        ...
        return new NextResponse(excelBuffer, {
            headers: {
                ...
                'Content-Disposition': `attachment; filename="${contestSlug}_leaderboard.xlsx"`,
